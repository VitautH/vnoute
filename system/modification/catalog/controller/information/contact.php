...
class ControllerInformationContact extends Controller {
	private $error = array();
				...
				public function callback(){
					if (
						isset($this->request->post['cb_name']) && 
						!empty($this->request->post['cb_name']) && 
						isset($this->request->post['cb_phone']) && 
						strlen($this->request->post['cb_phone']>4)
						)
					{
						$mesbody  = 'От: '.strip_tags($this->request->post['cb_name'])."\r\n";
						$mesbody .= 'Телефон: '.strip_tags($this->request->post['cb_phone'])."\r\n";
						$mesbody .= 'Сообщение: '.strip_tags($this->request->post['cb_message'])."\r\n";
						$mail = new Mail($this->config->get('config_mail'));
						$mail->setTo($this->config->get('config_email'));
						$mail->setFrom($this->request->post['cb_phone']);
						$mail->setSender($this->request->post['cb_name']);
						$mail->setSubject("Заказ обратного звонка от ".$this->request->post['cb_name']);
						$mail->setText($mesbody);
						$mail->send();
					}
				}
